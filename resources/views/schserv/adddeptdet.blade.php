...
<head>
      ...
      <title>add school profile </title>
                     ...
                     <div class="panel panel-bd lobidrag">
                        <div class="panel-heading">
                           <div class="btn-group" id="buttonexport">
                              <a href="#">
                                @foreach($skooldeet as $data)
                                 <h4> ADD further details  to "{{$data->skolssn}}"</h4>
                                 @endforeach
                              </a>
                           </div>
                        ...
                        <form method="post"  action='/skolprofprocess/{id}' enctype="multipart/form-data">
                           @csrf	 
                        @foreach($skooldeet as $datda)    
                            <div class="form-group ">
                              <label>School Name</label>
                              <input type="text"  class="form-control" name="skolssn" value="{{$datda->skolssn}}" disabled> 
                            </div> 
                     

                            <div class="form-group ">
                            <label>School Acronymn </label>
                            <input type="hidden" name="id" class="form-control"  value="{{$datda->id}}">
                            <input type="text" class="form-control"  value="{{$datda->skolsacr}}"disabled>
                            </div>
                        
               

                            <div class="form-group ">
                            <label>School Unit tittle</label>
                            <input type="text" class="form-control"  value="{{$datda->skolsh}}"disabled>
                            </div>



                            <div class="form-group ">
                              <label>School head fullname</label>
                              <input type="text"  class="form-control"  value="{{$datda->skolsfn}}"disabled>
                            </div>
                            @endforeach

                            <div class="form-group ">
                            <label>Location </label>
                            <input type="text" class="form-control" name="supoloc" >
                            </div>
                        
                            <div class="form-group ">
                            <label>Support Email </label>
                            <input type="text" class="form-control" name="supoemal" >
                            </div>


                            <div class="form-group ">
                            <label>Support Phone </label>
                            <input type="integer" class="form-control" name="supofon" >
                            </div>
                        

                            <div class="form-group ">
                              <label>About school </label>
                              <textarea class="form-control" id="body" name="abtskol" ></textarea>
                            </div>


                            <div class="form-group ">
                              <label>History of School</label>
                              <textarea class="form-control" id="body2"name="hiskol" ></textarea>
                            </div>
                            

                            <div class="form-group ">
                              <label>Deans Profile</label>
                              <textarea class="form-control" id="body3"name="deansprof" ></textarea>
                            </div>

                           <input type= "submit" value="Submit School"> <br>  <br>
                           </form>
                              ...
                              <table id="dataTableExample1" class="table table-bordered table-striped table-hover">
                                 <thead>
                                    <tr class="info">
                                        <th>Id</th>
                                        <th>School Name</th>
                                        <th>School Acronymn</th>
                                       <th>School  Head tittle</th>
                                       <th>School  Head fullname</th>
                                       <th>Support email</th> 
                                       <th>Support phone</th> 
                                       <th>Location</th>
                                       <th>About School</th> 
                                       <th>History of School</th>
                                       <th>Deans Profile</th> 
                                       <th>Edit</th>
                                       <th>Delete</th>
                                 ...
                                 <tbody>                             
                                    ...
                                    <tr>
                                    @forelse($skooldeet as $datda)
                                       <td>{{$counter+=1}}</td>
                                       <td>{{$datda->skolssn}}</td>
                                       <td>{{$datda->skolsacr}}</td>  
                                       <td>{{$datda->skolsh}}</td>  
                                       <td>{{$datda->skolsfn}}</td> 
                                       <td>{{$datda->supoemal}}</td> 
                                       <td>{{$datda->supofon}}</td>
                                       <td>{{$datda->supoloc}}</td> 
                                       <td>{!! $datda->abtskol !!}</td> 
                                       <td>{!! $datda->hiskol !!}</td> 
                                       <td>{!! $datda->deansprof !!}</td> 
                                       ...
                                       <td class="text-right">
                                        ...
                                        <td> <form method="GET" action='/skolprofdel/{id}'>  
                                        {{method_field('DELETE')}}
                                             @csrf
                              <input type="hidden" name="id" value="{{$data->id}}"> 
                           
                        <button type ="submit" name="delete" class="btn btn-danger btn-sm" onclick="return confirm( 'This deletes all inputs,Are you sure you want to DELETE?');"><i class="fa fa-trash-o"></i></button>
              </form>    
               ...
               <div id="checck" class="modal custom-modal fade" role="dialog">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        ...
                        <div class="modal-header">
                                <h5 class="modal-title">Edit School profile</h5>
                        ...
                        <div class="panel-body">
                           ...
                           <form method="post" action='/editskolprof/{id}' enctype="multipart/form-data">
                                  @csrf	
                        
                            <div class="form-group ">
                              <label>School Name</label>
                              <input type="text"  class="form-control" name="skolssn3" id="skolssn2" > 
                            </div> 

                            <div class="form-group ">
                            <label>School Acronymn </label>
                            <input type="hidden" name="id" class="form-control" id="id2"  >
                            <input type="text" class="form-control"name="skolsacr3"  id="skolsacr2">
                            </div>
                        
               
                            <div class="form-group ">
                              <label>School head fullname</label>
                              <input type="text"  class="form-control" name="skolsfn3" id="skolsfn2">

                           <div class="form-group ">
                            <label>School Unit tittle</label>
                            <input type="text" class="form-control" name="skolsh3"   id="skolsh2">
                            </div>
                     

                            <div class="form-group ">
                            <label>Location </label>
                            <input type="text" class="form-control" name="supoloc3"  id="supoloc2" >
                            </div>
                        
                            <div class="form-group ">
                            <label>Support Email </label>
                            <input type="text" class="form-control" name="supoemal3" id="supoemal2">
                            </div>


                            <div class="form-group ">
                            <label>Support Phone </label>
                            <input type="text" class="form-control" name="supofon3" id="supofon2" >
                            </div>
                        
                            @forelse($skooldeet as $datda)
                            <div class="form-group ">
                              <label>About school </label>
                              <textarea class="form-control" id="body4" name="abtskol3">{!! $datda->abtskol !!}</textarea>
                            </div>


                            <div class="form-group ">
                              <label>History of School</label>
                              <textarea class="form-control" id="body5" name="hiskol3">{!! $datda->hiskol !!}   </textarea>
                            </div>
                            

                            <div class="form-group ">
                              <label>Deans Profile</label>
                              <textarea class="form-control" id="body6" name="deansprof3"> {!! $datda->deansprof !!}  </textarea>
                            </div>
                            @endforeach
   ...
   <script src="{{asset('js/app.js')}}"></script>
    ...
    $(document).ready(function(){
      $('.upp').on('click', function(){
        ...
        console.log(data);
        $('#skolssn2').val(data[1]);
        $('#skolsacr2').val(data[2]);
        $('#skolsh2').val(data[3]);
        $('#skolsfn2').val(data[4]);
        $('#supoemal2').val(data[5]);
        $('#supofon2').val(data[6]);
        $('#supoloc2').val(data[7]);
        $('#abtskol2').val(data[8]);
        $('#hiskol2').val(data[9]);
        $('#deansprof2').val(data[10]);
        $('#id2').val(data[11]);
      });
...
<script>
    ...
    ClassicEditor.create(document.querySelector("#body5"), {
       extraPlugins: [SimpleUploadAdapterPlugin],
    }).catch((error) => {
       console.error(error);
    });

    ClassicEditor.create(document.querySelector("#body6"), {
       extraPlugins: [SimpleUploadAdapterPlugin],
    }).catch((error) => {
       console.error(error);
    });
